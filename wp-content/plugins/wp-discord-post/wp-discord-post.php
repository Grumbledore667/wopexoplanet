...
class WP_Discord_Post {
	...
	public function settings_init() {
		...
		add_settings_field(
			...
			esc_html__( 'Message Format', 'wp-discord-post' ),
		...
		register_setting( 'general', 'wp_discord_post_message_format' );
	}
	...
	public function print_message_format_field() {
		...
		echo '<textarea style="width:500px;height:150px;" name="wp_discord_post_message_format">' . esc_textarea( $value ) . '</textarea><br />';
		echo '<span class="description">' . esc_html__( 'Change the format of the message sent to Discord. The available placeholders are %title%, %author% and %url%. HTML is not supported.', 'wp-discord-post' ) . '</span>';
	}
	...
	public function send_post( $id, $post ) {
		...
		$content = str_replace(
			...
			array( esc_html( $post->post_title ), $author, get_permalink( $id ) ),
			$message_format
		);
		...
		$args = apply_filters( 'wp_discord_request_args', $args, $post );
